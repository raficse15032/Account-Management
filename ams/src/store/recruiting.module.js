            ...
            .then(response => {
                if(data.id==''){
                  commit(SET_ADD_VISA,response.data);
                    ...
                    iziToast.success({position:'topRight',title:'Ok',message:"Workers Status Updated Successsfully"})
                }
                resolve(response);
            })
            .catch(function(error) {
...
const mutations = {
    ...
    [SET_ADD_VISA](state,data){
        state.visas.data.push(data);
        iziToast.success({position:'topRight',title:'Ok',message:"Workers Info Add Successsfully"})
    },
